...
boolean  WiFiManager::startConfigPortal(char const *apName, char const *apPassword) {
    ...
    Serial.println("Config Portal Running, blocking, waiting for clients...");
...
bool WiFiManager::setSTAConfig(){
    ...
    DEBUG_WM(DEBUG_DEV,F("Gateway:"),_sta_static_gw);
    DEBUG_WM(DEBUG_DEV,F("Subnet Mask:"),_sta_static_sn);
    #endif
    preferences_wifi.begin("staticIP", false);
    String ip = preferences_wifi.getString("ip", "");
    String gw = preferences_wifi.getString("gw", "");
    String sn = preferences_wifi.getString("sn", "");
    optionalIPFromString(&_sta_static_ip, ip.c_str());
    optionalIPFromString(&_sta_static_gw, gw.c_str());
    optionalIPFromString(&_sta_static_sn, sn.c_str());
    preferences_wifi.end();
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  if (scan) {
    DEBUG_WM(DEBUG_VERBOSE,F("[handleWifi] scan"));
    #ifdef WM_DEBUG_LEVEL
...
void WiFiManager::handleClose(){
  ...
  if (SetMode(true))
  {
    Serial.println(" [IMPORTANT] From now on this sensor will work in StandAlone mode.");
    reboot();
  }
...
String WiFiManager::WiFi_psk(bool persistent) const {
  ...
  void WiFiManager::WiFiEvent(WiFiEvent_t event,system_event_info_t info){
    ...
    //#ifdef WM_DEBUG_LEVEL
      ...
      {
        DEBUG_WM(DEBUG_VERBOSE,F("[EVENT] WIFI_REASON: WIFI_REASON_4WAY_HANDSHAKE_TIMEOUT"));
        WiFi.reconnect();
      }
    //#endif
